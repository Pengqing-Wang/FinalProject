...
public class USD_to_CAD extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_usd_to__cad);
        Button convert = findViewById(R.id.convertButton);
        convert.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                EditText numEntered = (EditText) findViewById(R.id.cadenter);
    ...
    void processResponse(final JSONObject response, final Double amountEntered) {
        try {
            Double exchangeRate = response.getJSONObject("quotes").getDouble("USDCAD");
            Double convertedAmount = amountEntered * exchangeRate;
            TextView cadoutput = (TextView) findViewById(R.id.cadoutput);
            cadoutput.setText(convertedAmount.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
